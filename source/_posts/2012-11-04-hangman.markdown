...
i.e., from text to audio or image. Language model is usually used for state transition probability
distribution.

In our case, the hidden states are the English words in the answer sentences, e.g., `it's`, `a`,
...
English words that matches the half-revealed pattern, e.g., normalized $P(day), P(pay), P(gay), P(hey),
..., etc$. for state `__y`, and zero otherwise. With the transition and emission probabilities defined, we
...
stage	|errors	|hint	| HMM predicts	| guess
-----	|-------	|----	|-------	| -----
...
stage	|errors	|hint	| HMM predicts	| guesses
-----	|-------	|----	|-------	| -----
